...
# Initialise fields: magnetic, veclocity, and charge density
    if I_Tech['REAL'] == 'float':
        field_u_init = np.zeros((I_Tech['num_nodes_pad'], 4), dtype=np.float32)
        field_b_init = np.zeros((I_Tech['num_nodes_pad'], 4), dtype=np.float32)
        field_rho_init = np.ones((I_Tech['num_nodes_pad'], 1), dtype=np.float32)
    else:
        field_u_init = np.zeros((I_Tech['num_nodes_pad'], 4), dtype = np.float64)
        field_b_init = np.zeros((I_Tech['num_nodes_pad'], 4), dtype = np.float64)
...
# specified testcase.
    ...
    u_max = max(np.sqrt(np.power(field_u_init[:, 0], 2) + np.power(field_u_init[:, 1],2) + np.power(field_u_init[:, 2],2)))
...
# switch time_integrator_num_fields
    if time_integrator_num_fields == 2:
        ...
        'field_divB': field_divB, \
            ...
            raise Exception('Wrong value of time_integrator_num_fields =' + str( time_integrator_num_fields))
