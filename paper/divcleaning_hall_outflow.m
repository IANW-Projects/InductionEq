...
% discretization forms of interest for the nonlinear induction equation and
% computes the energy of the solution and the divergence error over time. Only
% outflow boundary conditions are used and the Hall term is included.
% As initial condition, an analytical solution of the nonlinear induction
% equation is used (testcase: hall_periodic).
...
io = fopen('hall_periodic_divcleaning.txt', 'w');
    ...
    for div_cleaning = [true, false]
      for div_cleaning_form = div_cleaning_forms
        ...
        if div_cleaning
          ...
          filename = sprintf('hall_periodic_N_%d_order_%d_%s_%s_%s__DC_1_%s.h5', ...
                              N, order, lower(form_uiBj(10:end)), lower(form_source(12:end)), ...
                              lower(form_ujBi(10:end)), DC_label);
        else
          filename = sprintf('hall_periodic_N_%d_order_%d_%s_%s_%s__DC_0.h5', ...
                              N, order, lower(form_uiBj(10:end)), lower(form_source(12:end)), ...
        ...
        fprintf(io, '%d, %d, %s, %s, %s, %.15e, %.15e, %.15e, %.15e, %d, %s \n', ...
              ...
              I_Results('runtime'), I_Results('energy'), 0, I_Results('divergence_norm'), div_cleaning, DC_form_output);
